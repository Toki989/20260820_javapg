...
class Todo { // ★変更 Todo（1件分のやること）の設計図です
    ...
    public String getMemo() { // メモを読み出すメソッドです
        return memo; // メモを返します
    } // getMemoの終わりです
...
public class App { // Appという名前のプログラムを定義します
        ...
        for (String line : Files.readAllLines(path, StandardCharsets.UTF_8)) { // ★追加 UTF-8で各行を読み込みます
            try { // 壊れた行があってもほかのTodoを読み込めるようにします
                ...
                if (line.startsWith("v3,")) { // メモを含む最新形式かを調べます
                    ...
                    String memo = new String(Base64.getUrlDecoder().decode(values[6]), StandardCharsets.UTF_8); // 日本語メモを元に戻します
                    todos.add(new Todo(id, title, done, values[4], memo, updatedAt)); // 最新形式のTodoをListへ戻します
                } else if (line.startsWith("v2,")) { // 締切日と更新時刻を持つ旧形式かを調べます
                    ...
                    long updatedAt = Long.parseLong(values[3]); // 更新時刻を読み取ります
                    String title = new String(Base64.getUrlDecoder().decode(values[5]), StandardCharsets.UTF_8); // 日本語タイトルを元に戻します
                    todos.add(new Todo(id, title, done, values[4], "", updatedAt)); // メモなしの旧形式TodoをListへ戻します
                } else { // 従来の保存形式の場合です
                    ...
                    todos.add(new Todo(id, values[2], done, "", "", id)); // 従来データを締切日・メモなしでListへ戻します
                } // 保存形式ごとの読み込みを終えます
    ...
    static String validDeadlineOrEmpty(String deadline) { // 締切日を保存できる形へ整えます
        if (deadline.isBlank()) { // 未入力かを調べます
            return ""; // 未入力は締切日なしとして返します
        } // 未入力確認を終えます
        try { // 正しい日付かを確認します
            LocalDate.parse(deadline); // 年月日の形式として解析します
            return deadline; // 正しい日付をそのまま返します
        } catch (RuntimeException e) { // 正しくない日付を受け止めます
            return ""; // 不正な値は締切日なしとして返します
        } // 日付確認を終えます
    } // 締切日の整形を終えます
    ...
    static String renderPage(String sort, YearMonth month) { // Todo画面全体のHTMLを作ります
        ...
        for (Todo todo : displayedTodos) { // 表示順にTodoを取り出します
            ...
            html.append("<label>期限<input type='date' name='deadline' value='").append(escapeHtml(todo.getDeadline()))
    ...
    public static void main(String[] args) throws IOException { // プログラムの開始地点です
        ...
        server.createContext("/", exchange -> { // 「/」へのアクセスを受け取ったときの処理です
            ...
            if (path.equals("/add") && exchange.getRequestMethod().equals("POST")) { // ：フォームから「/add」へ送られたPOSTかを調べます
                ...
                String deadline = validDeadlineOrEmpty(getParameter(formData, "deadline")); // 入力された締切日を検証します
            ...
            } else if (path.equals("/edit") && exchange.getRequestMethod().equals("POST")) { // 編集フォームから送られたPOSTを処理します
                String formData = new String(exchange.getRequestBody().readAllBytes(), StandardCharsets.UTF_8); // フォームをUTF-8で読み取ります
                ...
                exchange.getResponseHeaders().set("Location", "/"); // 編集後の移動先を指定します
                exchange.sendResponseHeaders(303, -1); // 一覧へ移動するようブラウザへ伝えます
                exchange.close(); // この応答を閉じます
                return; // 通常ページの処理へ進まず終了します
            } else if (path.equals("/delete-completed") && exchange.getRequestMethod().equals("POST")) { // 完了済み一括削除を処理します
                if (todos.removeIf(Todo::isDone)) { // 完了済みTodoをすべて取り除き、変更があったか調べます
                    save(); // 削除後の全件をすぐに保存します
                } // 保存が必要かの確認を終えます
                exchange.getResponseHeaders().set("Location", "/"); // 削除後の移動先を指定します
                exchange.sendResponseHeaders(303, -1); // 一覧へ移動するようブラウザへ伝えます
                exchange.close(); // この応答を閉じます
                return; // 通常ページの処理へ進まず終了します
